...
class Model_report extends CI_Model
{
    ...
    {
        $this->db->select('year(invoice.batas_bayar) as Tahun,month(invoice.batas_bayar) as Bulan, kategori.nama_kategori as namakategori,SUM(orders.jumlah) as Terjual, SUM(orders.harga) as total_harga, SUM(orders.jumlah*orders.harga) as penghasilan, toko.nama_toko as namatoko, SUM(orders.jumlah*orders.harga) as penghasilannamatoko');
        $this->db->from('invoice');
        $this->db->join('orders', 'orders.id_invoice = invoice.id');
    ...
    {
        $this->db->select('year(invoice.batas_bayar) as Tahun,month(invoice.batas_bayar) as Bulan, orders.nama_barang, orders.harga, kategori.nama_kategori, toko.nama_toko, orders.harga as penghasilan, sum(orders.jumlah) as terjual, barang.stok_barang');
        $this->db->from('invoice');
        $this->db->join('orders', 'orders.id_invoice = invoice.id');
        $this->db->join('barang', 'barang.id_barang = orders.id_barang');
        $this->db->join('kategori', 'kategori.id = barang.id_kategori');
        ...
        $this->db->where('invoice.batas_bayar >=', $dari);
        $this->db->where('invoice.batas_bayar <=', $sampai);
        $this->db->group_by('orders.nama_barang,month(batas_bayar), year(batas_bayar)');
        $this->db->order_by('year(invoice.batas_bayar)', 'ASC');
        return $this->db->get();
    }
    public function customer_result1($toko)
    {
        ...
        $this->db->group_by('email, month(batas_bayar), year(batas_bayar)');
    ...
    {
        $this->db->select('invoice.nama_pemesan, invoice.email, invoice.hp_pemesan, COUNT(invoice.email) as Jumlah_Transaksi, toko.nama_toko, year(invoice.batas_bayar) as Tahun,month(invoice.batas_bayar) as Bulan,');
        $this->db->from('invoice');
        $this->db->join('toko', 'toko.id = invoice.id_toko');
        $this->db->where('toko.id =', $toko);
        $this->db->where('invoice.batas_bayar >=', $dari);
    ...
    public function kategori($id_toko)
    {
        ...
        foreach ($query->result() as $k) {
            $output .= '<option value="' . $k->id . '">' . $k->nama_kategori . '</option>';
        }
        return $output;
    }
}
